...
class ModelRendererEditor : Editor {
  ...
  public override void OnInspectorGUI() {
    ...
    EditorGUILayout.HelpBox(
        "Initial Rotation should make sure palm faces forward, fingers points up in global axis.",
        MessageType.Info);
    EditorGUILayout.HelpBox(
        "Global rotation of Transform component is locked to use value of Initial Rotation",
        MessageType.None);
    ...
    GUILayout.Space(5);
    if (GUILayout.Button("Auto detect properties")) AutoDetect(target as ModelRenderer);
    EditorGUILayout.HelpBox(
        "Please manually check if auto detect set fields correctly. Please refer to help page for details.",
        MessageType.Info);
  }
