...
class HandRenderer : MonoBehaviour {
  ...
  void Update() {
    ...
    if (hand == null) {
      foreach (var p in points) p.SetActive(false);
      foreach (var l in links) l.SetActive(false);
      if (colliderObject != null) colliderObject.SetActive(false);
      return;
    }
    ...
    for (int i = 0; i < points.Count; i++) {
      var go = points[i];
      go.transform.localPosition = handWorld2LocalMatrix.MultiplyPoint(hand.points[i]);
      go.SetActive(true);
    }
