...
class HandDisplaySwitch : MonoBehaviour {
  ...
  void Awake() {
    if (Hands == null) GameObject.Destroy(this);
  ...
  IEnumerator Start() {
    ...
    if (!GestureProvider.HaveSkeleton) Hands = Hands.Where(o => !o.isModel).ToArray();
    if (index > Hands.Length) index = 0;
    if (Hands.Length > 0) SwitchDisplay(false);
    if (Hands.Length <= 1) GameObject.Destroy(this);
  }

  void Update() {
    if (Input.GetKeyDown(KeyCode.Space)) SwitchDisplay();
  }

  void SwitchDisplay(bool next = true) {
    if (next) index = (index + 1) % Hands.Length;
    for (int i = 0; i < Hands.Length; i++) Hands[i].HandsRoot.SetActive(i == index);
  }

  public void OnStateChanged(int state) {
    if (state == 1) SwitchDisplay();
  }
}
