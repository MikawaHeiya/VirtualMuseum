...
class HandStateChecker : MonoBehaviour {
  ...
  void Update() {
    ...
    } else if ((state != 0 || CanSkipPrepare) &&
               IsFlagMatch(LeftFlag, RightFlag, TriggerCondition)) {
  ...
  HandFlag GetCustomFlag(bool isLeft) {
    ...
    if (CustomGestureProvider.Current == null) return flag;
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (isLeft ? producer.IsLeftMatch : producer.IsRightMatch) flag |= current;
    ...
    for (int i = 0; i < size; i++) {
      if (CustomGestureProvider.Current.DualHandCustomGestures[i].IsMatch) flag |= current;
      current = (HandFlag)(((int)current) << 1);
    }
    return flag;
  }
  ...
  void SetState(int newState) {
    if (state != newState && OnStateChanged != null) OnStateChanged.Invoke((int)newState);
    state = newState;
  }
}
