...
[CreateAssetMenu(fileName = "CustomGesture",
                 menuName = "ViveHandTracking/Custom Gesture (Dual Hand)", order = 251)]
public class DualHandGestureProducer : BaseDualHandGestureProducer {
  ...
  public override void CheckGesture() {
    ...
    if (!leftCondition.CheckHandMatch(GestureProvider.LeftHand,
                                      CustomGestureProvider.LeftHandState))
      return;
    if (!rightCondition.CheckHandMatch(GestureProvider.RightHand,
                                       CustomGestureProvider.RightHandState))
      return;
